    ...
    for ann in sorted(os.listdir(ann_dir)):
        ...
        for elem in tree.iter():
            if 'filename' in elem.tag:
                all_imgs += [img]
                img['filename'] = ann_dir[:-1] + '/' + elem.text
...
def parse_annotation_new(ann_dir, img_dir, labels=[]):
    all_imgs = []
    seen_labels = {}
...
def parse_annotation_txt(ann_dir, img_dir, labels=[]):
    ...
    for ann in sorted(os.listdir(ann_dir)):
        img = {'object':[]}
        file_ann = open(ann_dir+ann, "r")
        img['filename'] = img_dir + str(ann).split('.')[0]+'.png'
        img['width'] = 512
        img['height'] = 512
        img['object'] = []
        ...
        for line in file_ann:
            ...
            obj['name'] = 'RBC'
            dimSplit = line.split()
            obj['xmin'] = int(round(float(dimSplit[0])))
            obj['ymin'] = int(round(float(dimSplit[1])))
            obj['xmax'] = int(round(float(dimSplit[2])))
            obj['ymax'] = int(round(float(dimSplit[3])))
