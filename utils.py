...
#It takes in boxes with xmin,ymin,xmax,ymax    
def bbox_iou_vals(box1,box2):
    x1_min  = box1[0]
    x1_max  = box1[2]
    y1_min  = box1[1]
    y1_max  = box1[3]
...
def find_nearest_box(box1,boxes,image):
    nearest=-1
    min_dist=100000
    for box in boxes:
        #x_val = int(box.x*image.shape[1])
        #y_val = int(box.x*image.shape[0])
        val = (box1.x-box.x)**2 + (box1.y-box.y)**2
        dist = math.sqrt(val)
        if(dist< min_dist):
            min_dist=dist
            nearest = box
...
def draw_boxes(image, boxes, labels, h_threshold, w_threshold, h_min, w_min,gt):
    final_boxes=[]    
    for box in boxes:
        if(box.h > h_threshold or box.w > w_threshold or box.h < h_min or box.w < w_min):
            continue
        ...
        final_boxes.append(BoundBox(c_x, c_y, w, h))
        cv2.rectangle(image, (xmin,ymin), (xmax,ymax), (255,0,0), 2)
        cv2.putText(image, str(box.get_score()), 
                    (xmin, ymin + 16), 
                    0, 
                    1e-3 * image.shape[0], 
                    (255,0,0), 2)
